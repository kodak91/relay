...
//
// 분기 기간 스냅샷으로 태스크/티켓/채팅/회의 데이터를 읽어 5개 항목(가중치) 평가 리포트 생성.
...
평가 규칙:
- 반드시 제공된 데이터(태스크/티켓/채팅/회의)에 근거하라. 데이터가 부족한 항목은 추측하지 말고 "데이터 부족"으로 명시하고 보수적으로(중간점 부근) 평가하라.
...
# INSA 분기 평가 — {이름} | {분기}
> 평가 기간: {기간}
> 데이터 출처: 채팅 N건 / 태스크 N건 / 티켓 N건 / 회의 N회
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [messages, tasks, tickets, meetings] = await Promise.all([
      ...
      fsQueryRange(`projects/${projectId}`, 'tickets', periodStart, periodEnd, idToken),
    ...
    // 태스크: 본인 담당
    const myTasks = tasks.filter((t) => t.assigneeUid === memberId || t.ownerUid === memberId || (memberName && t.assigneeName === memberName));
    const taskLines = myTasks.map((t) => `[${t.done ? '완료' : '진행'}] ${t.title || t.text || ''}${t.due ? ` (마감:${t.due})` : ''}`);

    // 티켓: 본인 담당
    const myTickets = tickets.filter((t) => t.assigneeUid === memberId || (memberName && t.assigneeName === memberName));
    const ticketLines = myTickets.map((t) => `[${t.status || '열림'}] ${t.ticketTitle || t.title || ''}${t.ticketPriority || t.priority ? ` · ${t.ticketPriority || t.priority}` : ''}`);
    ...
    const evidence = { chat: myMessages.length, tasks: myTasks.length, tickets: myTickets.length, meetings: myMeetings.length };
    ...
    userPrompt += `\n## 데이터 출처 요약\n채팅(본인) ${evidence.chat}건 / 태스크 ${evidence.tasks}건 / 티켓 ${evidence.tickets}건 / 회의 ${evidence.meetings}회\n`;
    userPrompt += `\n## 채팅 (본인 + 관리자 메시지, ${chatLines.length}건)\n${chatLines.join('\n') || '데이터 없음'}\n`;
    userPrompt += `\n## 태스크 (${taskLines.length}건)\n${taskLines.join('\n') || '데이터 없음'}\n`;
    userPrompt += `\n## 티켓 (${ticketLines.length}건)\n${ticketLines.join('\n') || '데이터 없음'}\n`;
    ...
    const totalDataPoints = evidence.chat + evidence.tasks + evidence.tickets + evidence.meetings;
